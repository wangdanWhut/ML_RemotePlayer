...
@interface MLResourceLoader ()<MLAudioDownLoaderDelegate>

@property (nonatomic, strong) MLAudioDownLoader *downLoader;
...
@implementation MLResourceLoader

- (MLAudioDownLoader *)downLoader {
    if(!_downLoader) {
        _downLoader = [[MLAudioDownLoader alloc] init];
...
//        }
        ...
        NSData *data = [NSData dataWithContentsOfFile:[MLAudioFileTool tmpPathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data.length == 0) {
            data = [NSData dataWithContentsOfFile:[MLAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
//    }
    ...
    if ([MLAudioFileTool isCacheFileExists:url])
    {
        // 三个步骤, 直接响应数据
...
- (void)handleRequestWithLoadingRequest: (AVAssetResourceLoadingRequest *)loadingRequest {
    NSURL *url = [loadingRequest.request.URL httpURL];
    // 1. 填充信息头
    loadingRequest.contentInformationRequest.contentType = [MLAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = [MLAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    
    // 2. 响应数据
    NSData *data = [NSData dataWithContentsOfFile:[MLAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
